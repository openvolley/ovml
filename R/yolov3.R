...
#' @param version integer: currently only 3, for YOLO v3
...
ovml_yolo <- function(version = 3, device = "cpu", weights_file = "auto") {
    assert_that(is.numeric(version), version %in% c(3))
    ...
    if (length(weights_file) && nzchar(weights_file) && !is.na(weights_file)) {
        if (identical(tolower(weights_file), "auto")) {
            ...
            if (!file.exists(weights_file)) download.file("https://pjreddie.com/media/files/yolov3.weights", destfile = weights_file, mode = "wb")
        }
